...
/**
 * As networkstatus_get_param(), but check anonrc_value before checking the
 * consensus. If anonrc_value is in-range, then return it instead of the
 * value from the consensus.
 */
int32_t
networkstatus_get_overridable_param(const networkstatus_t *ns,
                                    int32_t anonrc_value,
                                    const char *param_name,
                                    int32_t default_val,
                                    int32_t min_val, int32_t max_val)
{
  if (anonrc_value >= min_val && anonrc_value <= max_val)
    return anonrc_value;
  else
