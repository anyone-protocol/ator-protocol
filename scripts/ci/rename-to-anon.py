...
# Find all files that are just pure renames, and redo the renames from "tor" to "anon"
git_find_renames_command = ["git", "--no-pager", "diff", "--name-status", "--diff-filter=R", "-M100%", "last-commit-before-fork...main"]
...
for line in output_lines:
  ...
  if len(split_line) == 3:
    ...
    num_pure_renames += 1
print(f"Renamed {num_pure_renames} files back to their anon names.")
