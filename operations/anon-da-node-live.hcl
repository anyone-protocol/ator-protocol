...
job "anon-da-node-live" {
  ...
  group "dir-auth-eu-live-group" {
    # NB: fal-net-auth-1 (eu-central), hel-net-auth-1 (eu-east), nur-net-auth-1 (eu-central)
    count = 3
    constraint {
      attribute = "${meta.zone}"
      value = "eu"
  ...
  group "dir-auth-us-live-group" {
    # NB: ash-net-auth-1 (us-east), ash-net-auth-2 (us-east), hil-net-auth-1 (us-west), hil-net-auth-2 (us-west)
    count = 4
    constraint {
      attribute = "${meta.zone}"
      value = "us"
    ...
    task "sbws-bandwidth-puller" {
      ...
      template {
        data = <<-EOF
        V3BW_URL="{{- range service "sbws-bandwidth-us-central" }}http://{{ .Address }}:{{ .Port }}/latest.v3bw{{- end }}"
        EOF
        destination = "secrets/env"
        env         = true
      }
